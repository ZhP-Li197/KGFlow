...
class StructuredInferenceWithDB(BaseInferenceWithDB):
    def __init__(
        self, extractor, aligner, triplets_db, triplet_extraction_only: bool = False
    ):
        self.extractor = extractor
        self.aligner = aligner
        self.triplets_db = triplets_db
        # 为 True 时只保留 extract_triplets_from_text，不跑类型/关系/实体名等 ontology 后续 LLM
        self.triplet_extraction_only = triplet_extraction_only
    ...
    def extract_triplets_with_ontology_filtering(
        ...
        extracted_triplets = self.coerce_extracted_triplets_payload(
            self.extractor.extract_triplets_from_text(text), logger
        )
        ...
        if self.triplet_extraction_only:
            for t in initial_triplets:
                final_triplets.append(t.copy())
            return (
                initial_triplets,
                final_triplets,
                filtered_triplets,
        ...
        for triplet in extracted_triplets["triplets"]:
            ...
            try:
                ...
                if backbone_triplet_valid:
                    ...
                    logger.log(
                        logging.INFO, "Refined relation: %s" % (str(refined_relation))
                    )
                    logger.log(
                        logging.INFO,
