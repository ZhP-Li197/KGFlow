...
class BaseInferenceWithDB:
    ...
    @staticmethod
    def coerce_extracted_triplets_payload(
        extracted: Any, log: logging.Logger
    ) -> Dict[str, List[Any]]:
        """
        LLM 输出解析失败时 extract_triplets_from_text 可能返回 str 等非 dict；
        统一成 {\"triplets\": [...] }，避免下游 string indices 崩溃。
        """
        if not isinstance(extracted, dict):
            snippet = extracted
            if isinstance(extracted, str) and len(extracted) > 400:
                snippet = extracted[:400] + "…"
            log.error(
                "extract_triplets_from_text 返回非 dict（%s），本条文本按零三元组处理。片段: %r",
        ...
        if not isinstance(triplets, list):
            log.error(
                "extract_triplets_from_text 的 triplets 非 list（%s），本条文本按零三元组处理。",
                type(triplets).__name__,
            )
            return {"triplets": []}
        kept: List[Any] = []
        dropped = 0
        ...
        if dropped:
            log.error(
                "triplets 列表中含 %d 个非 dict 元素，已丢弃以免单条解析拖垮后续 refine",
                dropped,
            )
        return {"triplets": kept}
