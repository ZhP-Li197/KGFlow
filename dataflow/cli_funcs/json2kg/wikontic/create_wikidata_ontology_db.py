...
def resolve_contriever_model_path():
    ...
    repo_model_path = Path(__file__).resolve().parents[2] / "facebook" / "contriever"
...
def load_contriever():
    model_path, local_files_only = resolve_contriever_model_path()
    load_kwargs = {}
    ...
    logger.info(f"Loading Contriever model from: {model_path}")
    try:
        ...
        logger.warning(
            "Failed to load local Contriever model from %s, falling back to Hugging Face Hub",
            model_path,
        )
        fallback_kwargs = {}
        if hf_token:
...
def ensure_contriever_loaded():
    global tokenizer, model
    if tokenizer is None or model is None:
        tokenizer, model = load_contriever()


def resolve_mappings_dir(mappings_dir: str | None) -> Path:
    default_relative_dir = Path("utils") / "ontology_mappings"
    script_relative_dir = Path(__file__).resolve().parent / "utils" / "ontology_mappings"

    candidate_paths = []
    if mappings_dir:
        provided_path = Path(mappings_dir).expanduser()
        candidate_paths.append(provided_path)
        if not provided_path.is_absolute():
            candidate_paths.append(Path.cwd() / provided_path)
            candidate_paths.append(Path(__file__).resolve().parent / provided_path)
    else:
        candidate_paths.extend(
            [
                script_relative_dir,
                Path.cwd() / default_relative_dir,
            ]
        )
    ...
    raise FileNotFoundError(
        f"Could not find ontology mappings directory. Checked: {searched_paths}"
    )
...
def create_indexes(db):
    ...
    if "triplets" not in db.list_collection_names():
...
def create_wikidata_ontology_database(
    ...
    mappings_path = resolve_mappings_dir(mappings_dir)

    logger.info("Starting database population process")
    logger.info(f"Using database: {database}")
    logger.info(f"Loading mapping files from: {mappings_path}")
