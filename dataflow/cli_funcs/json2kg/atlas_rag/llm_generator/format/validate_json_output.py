...
def fix_triple_extraction_response(response: str, **kwargs) -> str:
    ...
    for idx, item in enumerate(parsed_objects):
        ...
        # Drop keys that are not declared by the schema. This avoids
        # jsonschema additionalProperties failures on noisy model outputs,
        # e.g. {"Head": ..., "Relation": ..., "Tail": ..., "see": ...}.
        corrected_item = {k: v for k, v in corrected_item.items() if k in required_keys_set}
        
        # Check for missing keys in corrected_item
        missing = required_keys_set - set(corrected_item.keys())
        if missing:
