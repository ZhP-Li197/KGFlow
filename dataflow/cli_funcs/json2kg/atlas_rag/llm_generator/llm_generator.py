...
class LLMGenerator():
    ...
    def get_runtime_metrics(self):
        ...
        metrics["retry_count"] = self.retry_count
        calls = metrics["api_calls"]
        metrics["api_avg_time"] = (metrics["api_total_time"] / calls) if calls > 0 else 0.0
        return metrics
    ...
    def _api_inference(self, message, config: Optional[GenerationConfig] = None,
        ...
        with self._metrics_lock:
            self._runtime_metrics["api_calls"] += 1
            self._runtime_metrics["inflight"] += 1
            self._runtime_metrics["inflight_peak"] = max(
                self._runtime_metrics["inflight_peak"],
                self._runtime_metrics["inflight"]
            )
        ...
        try:
            ...
            with self._metrics_lock:
                self._runtime_metrics["inflight"] = max(0, self._runtime_metrics["inflight"] - 1)
                self._runtime_metrics["api_total_time"] += elapsed
                self._runtime_metrics["api_max_time"] = max(self._runtime_metrics["api_max_time"], elapsed)
                if self._runtime_metrics["api_min_time"] is None:
                    self._runtime_metrics["api_min_time"] = elapsed
                else:
                    self._runtime_metrics["api_min_time"] = min(self._runtime_metrics["api_min_time"], elapsed)
                if request_success:
                    self._runtime_metrics["api_success"] += 1
                else:
                    self._runtime_metrics["api_failures"] += 1
