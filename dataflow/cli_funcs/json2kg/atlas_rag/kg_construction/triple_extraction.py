...
class KnowledgeGraphExtractor:
    ...
    def run_extraction(self):
        ...
        if self.config.profile_mode:
            effective_workers = getattr(self.model, "max_workers", "N/A")
            print(
                "[profile] start extraction "
                f"batch_size_triple={self.config.batch_size_triple}, "
                f"config.max_workers={self.config.max_workers}, "
                f"model.max_workers={effective_workers}"
            )
            if hasattr(self.model, "reset_runtime_metrics"):
                self.model.reset_runtime_metrics()
            ...
            with open(output_file, "w", encoding="utf-8") as output_stream:
                for batch in data_loader:
                    ...
                    for key in result_keys:
                        ...
                        retry_before = getattr(self.model, "retry_count", 0)
                        ...
                        retry_after = getattr(self.model, "retry_count", 0)
                        stage_metrics[key] = {
                            "seconds": stage_elapsed,
                            "retries": max(0, retry_after - retry_before),
                            "requests": len(messages_dict[key]),
                        }
                    ...
                    if self.config.profile_mode:
                        ...
                        stage_summary = ", ".join(
                            [
                                f"{k}: {v['seconds']:.2f}s/{v['requests']}req/{v['retries']}retry"
                                for k, v in stage_metrics.items()
                            ]
                        )
                        print(
                            f"[profile][batch {batch_counter}] size={len(batch_ids)} "
                            f"total={batch_elapsed:.2f}s | {stage_summary}"
                        )
                        if hasattr(self.model, "get_runtime_metrics"):
                            metrics = self.model.get_runtime_metrics()
                            print(
                                "[profile][runtime] "
                                f"calls={metrics['api_calls']}, ok={metrics['api_success']}, "
                                f"fail={metrics['api_failures']}, retry={metrics['retry_count']}, "
                                f"inflight_peak={metrics['inflight_peak']}, "
                                f"avg_api={metrics['api_avg_time']:.2f}s, "
                                f"max_api={metrics['api_max_time']:.2f}s"
                            )
        ...
        if self.config.profile_mode and hasattr(self.model, "get_runtime_metrics"):
            metrics = self.model.get_runtime_metrics()
            min_api = metrics["api_min_time"]
            min_api_text = f"{min_api:.2f}s" if min_api is not None else "N/A"
            print(
                "[profile][final] "
                f"calls={metrics['api_calls']}, ok={metrics['api_success']}, "
                f"fail={metrics['api_failures']}, retry={metrics['retry_count']}, "
    ...
    def compute_kg_embedding(self, encoder_model, batch_size: int = 2048):
