...
def normalize_chat_api_url(api_base: str) -> str:
...
def _chat_once(prompt: str) -> str:
    ...
    api_key = config['APIConfig'].get('API_KEY', '')
    headers = {"Content-Type": "application/json"}
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}"

    resp = requests.post(
        normalize_chat_api_url(config['APIConfig']['API_BASE']),
        headers=headers,
        json={
            "model": MODEL_NAME,
...
def call_openai_json(prompt: str) -> Dict:
    try:
        ...
        print(f"TreeKG LLM call failed: {error}")
        return {}
